...
const SideMenu: React.FC = () => {
    ...
    useEffect(() => {
        updateProgressAndThumbs();
    }, [minPrice, maxPrice]);

    const updateProgressAndThumbs = () => {
        if (!rangeRef.current || !minThumbRef.current || !maxThumbRef.current) return;
        ...
        maxThumbRef.current.style.left = `${100 - maxPercent}%`;

        // Update tooltips
        const minTooltip = minThumbRef.current.querySelector('.tooltip');
        const maxTooltip = maxThumbRef.current.querySelector('.tooltip');
        if (minTooltip) minTooltip.textContent = `$${minPrice}`;
        if (maxTooltip) maxTooltip.textContent = `$${maxPrice}`;
    };

    const handlePriceInput = (e: ChangeEvent<HTMLInputElement>, type: 'min' | 'max') => {
    ...
    const handleRangeInput = (e: ChangeEvent<HTMLInputElement>, type: 'min' | 'max') => {
        const value = parseInt(e.target.value, 10);

        if (type === 'min') {
            const newVal = Math.min(value, maxPrice - priceGap);
            setMinPrice(newVal);
        } else {
            const newVal = Math.max(value, minPrice + priceGap);
            setMaxPrice(newVal);
        }
    };
                ...
                <div className="price-range p-2 bg-white rounded-xl border-gray-100">
                    ...
                    <div className="relative h-12">
                        ...
                        <div className="relative">
                            <div
                                ref={minThumbRef}
                                className="thumb-min absolute top-6 w-5 h-5 bg-teal-500 rounded-full shadow-lg cursor-pointer transform -translate-x-1/2 -translate-y-1/2 hover:scale-110 transition-transform z-10"
                                style={{ left: '0%' }}
                            >
                            ...
                            <div
                                ref={maxThumbRef}
                                className="thumb-max absolute top-6 w-5 h-5 bg-teal-500 rounded-full shadow-lg cursor-pointer transform -translate-x-1/2 -translate-y-1/2 hover:scale-110 transition-transform z-20"
                                style={{ left: '75%' }}
                            >
